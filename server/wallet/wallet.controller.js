...
exports.convertRcoinToDiamond = async (req, res) => {
  try {
    ...
    const coinResult = setting.rCoinForDiamond / setting.diamond;
    user.rCoin -= req.body.rCoin;
    user.diamond += parseInt(req.body.rCoin / coinResult);
    await user.save();

    const income = new Wallet();
    income.userId = user._id;
    income.diamond = parseInt(req.body.rCoin / coinResult);
    income.type = 1;
